...
인덱스(색인)
    ...
    *ROWID 는 데이터행이 저장된 파일번호/블록번호/슬롯번호를 포함하고 있다.
    인덱스는 테이블과 독립적으로 존재하고, 한번 생성된 인덱스는 자동으로 유지된다.

    인덱스 생성
        자동생성
            PRIMARY KEY, UNIQUE 제약조건이 적용된 컬럼은 자동으로 인데스(색인)가 생성된다.
        수동생성 
            WHERE절에 조건식으로 자주 등장하는 컬럼은 조회 성능을 향상시키기 위해서 
            해당 컬럼에 대한 인덱스를 생성 할 수 있다.

    인덱스 생성이 유리한 경우
        +WHERE절에 조건식으로 자주 등장하는 경우 인덱스로 생성하자.
        +컬럼이 매우 다양한 값을 포함하고 있다면 인덱스를 생성하자.
        +데이터가 매우 많은 테이블을 대상으로 데이터를 조회했을때 조회작업을 할 때마다 

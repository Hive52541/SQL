...
--부서 아이디, 부서명, 관리자 아이디, 관리자 이름, 소재지 아이디, 소재지 도시, 우편번호, 주소, 국가코드, 국가명을 포함한다.
...
/*
인라인 뷰
    SELECT문의 FROM절에 정의된 서브쿼리(SELECT문) 으로 생성되는 가상의 테이블이다.
    오라클의 데이터베이스 객체가 아니기 때문에 SQL문이 실행되는 동안만 잠깐 생겨났다가 사라지는 가상의 테이블이다.

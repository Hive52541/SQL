...
트랜잭션
    ...
    데이터 베이스의 상태를 변환시키는 하나의 논리적인 기능을 수행하기 위한 논리적인 작업의 단위다.
    예) 홍길동이 이순신에게 100만원을 이체한다.
        ...
        논리적인 작업의 단위(그룹, 묶음, 연산들) = 감소 UPDATE작업 + 증가 UPDATE작업 + 추가 INSERT 작업
        논리적인 작업의 단위를 구성하는 모든 SQL 명령이 성공적으로 완료되어야 하나의 작업 단위가 완료된것이고, 
        이 경우에 데이터베이스에 반영시킨다.

트랜잭션 관련 명령어 
    COMMIT
        논리적인 작업단위를 구성하는 모든 작업이 성공적으로 완료되었을 때, 
        데이터베이스의 트랜잭션 관리자에게
        해당 작업 단위내에서 실행했던 처리 결과를 영구적으로 데이터베이스에 반영시키게 한다.

    ROLLBACK        
        논리적인 작업단위를 구성하는 작업중에서 오류가 발생했을 때,
        데이터베이스의 트랜잭션 관리자에게
...
트랜잭션의 시작과 종료
    트랜잭션의 시작
        ...
        -COMMIT, ROLLBACK 명령이 실행될 때 기존 트랜잭션이 종료되고, 새로운 트랜잭션이 시작된다.
    트랜잭션의 종료되고

...
TOP-N 분석
    ...
    형식
        ...
        *ROWNUM은   SELECT절에서 사용할 경우, 추출하는 데이터에 1부터 시작하는 순번을 부여하는 용도로 사용된다.
                    WHERE절에서 사용할 경우, 추출한 데이터 중 일부만 가져오는 용도로 활용할 수 있다.
        *ROWNUM은 순번을 1부터 부여하는 가상컬럼이기 때문에 ROWNUM의 시작을 1이 아닌 값부터 추출하려 할 경우에는
                    데이터가 조회되지 않는다.
        *ORDER BY 절로 데이터를 정렬한 후 ROWNUM으로 몇 갱의 데이터만 추출해야 하는 경우, 
         ORDER BY절을 먼저 처리하도록 인라인 뷰를 만들고, 그 뷰를 감싸는 바깥쪽 SQL에서 ROWNUM 처리를 하자.            
...
--직원들을 급여순으로 정렬했을 때 급여를 가장 많이 받는 직원 5명(1등~5등)의 직원아이디, 이름, 급여, 직종아이디를
--조회하기
SELECT
    ROWNUM,
    EMPLOYEE_ID,
...
--아래코드는 TOP-N 분석은 아니다.
--직원들을 급여순으로 정렬했을 때 급여를 가장 많이 받는 직원 5(6등~10등)명의 직원아이디, 이름, 급여, 직종아이디를
--조회하기
...
/*
    페이징 처리시 조회범위 구하기
        ...
        페이지번호가 1페이지부터 시작하고, 한 페이지당 10개씩 데이터를 표현하는 경우
        시작위치 = (페이지 번호 -1)*10 +1
        종료위치 = 페이지 번호*10
    */

...
CONTACT(컬럼 혹은 표현식,컬럼 혹은 표현식) : 두 문자열을 연결해서 반환한다

LENGTH(컬럼 혹은 표현식) : 문자열의 길이를 반환한다.

SUBSTR(컬럼 혹은 표현식, 시작위치, 길이) : 문자열을 시작위치부터 길이만큼 잘라서 반환한다. 시작위치는 1부터
...
--60번 부서에 소속된 직원들의 FIRST_NAME 과 LAST_NAME을 연결해서 조회하기
SELECT
...
--SAMPLE_BOOKS 에서 저자이름을 조회하기.
--저자이름을 조회하고, 이름의 첫번째 글자도 조회하기
...
/*
    ...
    MONTHS_DETWEEN(날짜, 날짜) : 두 날짜 사이의 개월수를 반환한다.
    ...
    TRUNC(날짜) : 저장된 날짜의 모든 시간 정보를 0시 0분 0초로 바꾼 날짜가 반환된다.
    ROUND(날짜) : 지정된 날짜의 시간정보를 정오를 지나기 전이면 해당 날짜가 정오를 지났으면 하루 증가된 날짜가 반환된다. 시간정보는 0시 0분 0초다.
...
--오늘이 12월 31일이라고 가정하고, 2달후의 날짜를 조회하면 2월 28일 혹은 2월 29일이 반환된다.
SELECT
...
--현재 일자를 기준으로 7일전, 3일전, 3일후, 7일후 날짜를 조회하기
...
--오늘 기준으로 7일 동안의 주문내역을 조회하기(오늘은 포함되지 않음)
SELECT
    *
FROM
    ORDERS
WHERE
 ...
 --오늘 기준으로 1개월 동안의 주문 내역을 조회하기(오늘은 포함되지 않음)
    SELECT
        *
    FROM
        ORDERS
...
SELECT
 ...
 --오늘 기준으로 7일동안의 주문내역을 조회하기(오늘은 포홤되지않음)
    SELECT
        SYSDATE,
        SYSDATE + 1/24, --1 시간후
        SYSDATE + 3/24, --3 시간후
...
묵시적 변환
        ...
        * '2005/01/01'DLSK '2005/01-01'인 형식의 경우 자동으로 변환된다. 

...
--직종이 변경된 적 있는 직원과 변경된 적이 없는 직원을 모두 조회하기
--* 직종이 변경된 적이 없는 직원정보는 EMPLOYEES 테이블을 전체 조회하는 것으로 대신한다.
SELECT
    EMPLOYEE_ID,
    JOB_ID,
    DEPARTMENT_ID,
...
FROM
    EMPLOYEES
WHERE
    SALARY <= 5000 INTERSECT --급여를 5000이하로 받는 직원에 대한 조회결과
    SELECT
        EMPLOYEE_ID
    FROM
        JOB_HISTORY;

--직종이 변경된 적이 있는 직원에 대한 조회결과

--교집합을 구하는 집합연사자는 IN 서브쿼리 혹은 EXISTS 서브쿼리로 대신할 수 있다.
SELECT
    EMPLOYEE_ID,
    FIRST_NAME,
    DEPARTMENT_ID
...
/*
    EXISTS 연산자
        ...
        EXISTS (서브쿼리)에서 서브쿼리는 조건을 만족하는 데이터가 여러건이 있다고 하더라고 조건을 만족하는 1건만 찾으면 추가적인 검색을 진행하지 않는다.
        + 실제 프로젝트에서 특정 조건을 만족하는 지 여부를 묻는 로직이 많이 사용되는데,
        COUTN(*) 로 조건을 만족하는 행의 갯수를 조회하는 것은 SQL 실행 성능에 나쁘다.
...
--WHERE 컬럼 연산자 (서브쿼리): 컬럼값과 비교가능한 의미 있는 값을 서버쿼리가 제공해야 한다.
--WHERE 값 연산자   (서브쿼리): 제시된 값과 비교가능한 읭미있는 값을 서버쿼리가 제공해야 한다.
--WHERE EXISTS      (서브쿼리): 서브쿼리는 이미있는 값을 제공하지 않아도 된다.
--                               의미없는 상수값을 반환해도 된다.

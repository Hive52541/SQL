...
NUMBER
    해당 컬럼의 데이터 타입을 숫자형으로 지정한다
VARCHAR2
    해당 컬럼의 데이터 타입을 가변 길이문자형으로 지정한다
DATE
    해당 컬럼의 데이터 타입을 날짜형으로 지정한다
PRIMARY KEY
    해당 컬럼을 테이블에서 행을 식별하기 위한 값을 가지는 기본키 컬럼으로 지정한다
...
NOT NULL
 ...
 DEFAULT STSDATE
    새로운 행을 추가할때 해당 컬럼에 값을 지정하지 않으면 시스템의 현재날짜가 저장된다.
...
/*
    ...
    * 생략된 컬럼에는 NULL값이 저장되거나, 기본값이 저장된다.
...
INSERT INTO SAMPLE_BOOKS VALUES (
    14,
    '이펙티브 자바',
    '심재철',
    18000,
